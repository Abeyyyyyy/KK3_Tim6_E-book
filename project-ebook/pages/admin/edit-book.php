...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        $stmt = $conn->prepare("
            UPDATE books 
            SET title = ?, author = ?, description = ?, price = ?, category_id = ?, is_published = ?
            WHERE id = ?
        ");
        $stmt->execute([
            $title, $author, $description, $price, $category_id, $is_published, $book_id
        ]);
...
<div class="p-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600">Edit informasi buku</p>
    </div>
